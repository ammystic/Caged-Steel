...
function openProfilePage(close = false) {
    ...
    for (const item of items){
        ...
        container.innerHTML = `<div class="details"><span>${item.weapon.toUpperCase()} | ${item.skin}</span><span>${item.price}</span></div><img src="/img/skins/${folder}${item.weapon.replace(/ /g, "_")}/${formatted.toLowerCase()}.png">`
        ...
        for (const button of allSellBTNs){
            button.addEventListener("click", () => {
                ...
                new Modal(` You sold ${weapon.weapon} | ${weapon.skin}<div class="skinImage ${weapon.rarity.toLowerCase()}"> <img src="/img/skins/${folder}${weapon.weapon.replace(/ /g, "_")}/${formatted.toLowerCase()}.png"></div>Earned money: ${weapon.price}`)
            });
        }
}
